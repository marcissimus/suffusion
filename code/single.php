...
if ($post->post_type !== 'post') {
	...
	if (isset($suffusion_cpt_layouts[$post->post_type]['byline-contents'])) {
		$byline_contents = explode(',', $suffusion_cpt_layouts[$post->post_type]['byline-contents']);
		$suf_cpt_bylines_posted_by = in_array('posted-by', $byline_contents, true) ? 'show' : 'hide';
		$suf_cpt_bylines_comments = in_array('comments', $byline_contents, true) ? 'show' : 'hide';
		$suf_cpt_bylines_permalinks = in_array('permalink', $byline_contents, true) ? 'show' : 'hide';
		$suf_cpt_bylines_post_date = in_array('date', $byline_contents, true) ? 'show' : 'hide';
	}
	else {
...
?>
		...
		<p><?php esc_html_e('Sorry, no posts matched your criteria.', 'suffusion'); ?></p>
        </article><!--post -->
