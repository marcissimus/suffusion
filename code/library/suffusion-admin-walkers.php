...
class Suffusion_MM_Walker_Edit extends Walker_Nav_Menu {
		...
		<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes); ?>">
				<div class="menu-item-bar">
						<div class="menu-item-handle">
								<span class="item-title">
										...
										<span class="is-submenu" <?php echo $submenu_text; ?>><?php _e('sub item', 'suffusion'); ?></span>
								</span>
								<span class="item-controls">
										...
										<span class="item-order hide-if-js">
												...
												?>" class="item-move-up">
														<abbr title="<?php esc_attr_e('Move up', 'suffusion'); ?>">&#8593;</abbr>
												</a>
												|
												<a href="<?php
												echo wp_nonce_url(
												...
												?>" class="item-move-down">
														<abbr title="<?php esc_attr_e('Move down', 'suffusion'); ?>">&#8595;</abbr>
												</a>
										</span>
										...
										echo (isset($_GET['edit-menu-item']) && $item_id === $_GET['edit-menu-item']) 
												...
												: add_query_arg('edit-menu-item', $item_id, remove_query_arg($removed_args, admin_url('nav-menus.php#menu-item-settings-' . $item_id)));
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
								...
								<p class="suffusion-mm-custom description">
										<label for="suffusion-mm-cols-<?php echo $item_id; ?>">
												...
												<select id="suffusion-mm-cols-<?php echo $item_id; ?>" class="suffusion-mm-cols" name="suffusion-mm-cols[<?php echo $item_id; ?>]">
														<?php for ($i = 1; $i <= 8; $i++) { ?>
																<option value="<?php echo $i; ?>" <?php selected(get_post_meta($item_id, 'suf_mm_cols', true), $i); ?>>
																		<?php echo $i; ?>
																</option>
														<?php } ?>
												</select>
								...
								<p class="suffusion-mm-custom description">
										<label for="suffusion-mm-widget-height-<?php echo $item_id; ?>">
												...
												<select id="suffusion-mm-widget-height-<?php echo $item_id; ?>" name="suffusion-mm-widget-height[<?php echo $item_id; ?>]" class="edit-menu-item-widget-height">
														<option value="row-equal" <?php selected($widget_height, 'row-equal'); ?>>
																<?php _e('Equal height for a row', 'suffusion'); ?>
														</option>
														<option value="original" <?php selected($widget_height, 'original'); ?>>
																<?php _e('Original height', 'suffusion'); ?>
														</option>
														<option value="mason" <?php selected($widget_height, 'mason'); ?>>
																<?php _e('Auto-arrange', 'suffusion'); ?>
														</option>
												</select>
										</label>
								</p>
						...
						<div class="menu-item-actions description-wide submitbox">
								...
								); ?>"><?php _e('Remove', 'suffusion'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo $item_id; ?>" href="<?php echo esc_url(add_query_arg(['edit-menu-item' => $item_id, 'cancel' => time()], admin_url('nav-menus.php')));
	 ...
	 * @return string
	 */
	public function suffusion_sidebar_select(int $id): string {
		global $suffusion_mm_sidebar_count;
		...
		$html = '<select id="' . esc_attr($field_id) . '" name="' . esc_attr($name) . '" class="edit-menu-item-wareas">';
		$html .= '<option value=""></option>';

		for ($i = 1; $i <= $suffusion_mm_sidebar_count; $i++) {
				$value = "Mega Menu Widget Area $i";
				$html .= '<option value="mm-' . $i . '" ' . selected($selection, "mm-$i", false) . '>' . esc_html($value) . '</option>';
		}
		$html .= '</select>';
