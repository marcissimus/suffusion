...
if (isset($_SERVER['REQUEST_METHOD']) && 
    ...
    $login = wp_signon([
        ...
        'remember' => esc_attr($_POST['remember-me'])
    ], false);
}
...
if (is_user_logged_in()) {
	...
	printf(
		__('You are currently logged in as <a href="%1$s" title="%2$s">%2$s</a>.', 'suffusion'),
		get_author_posts_url($login->ID),
		esc_attr($login->display_name)
	);
?>
	<a href="<?php echo wp_logout_url(get_permalink()); ?>" 
	   title="<?php _e('Log out', 'suffusion'); ?>">
		<?php _e('Log out', 'suffusion'); ?>
	</a>
<?php
}
	...
	printf(
		__('You have successfully logged in as <a href="%1$s" title="%2$s">%2$s</a>.', 'suffusion'),
		get_author_posts_url($login->ID),
		esc_attr($login->display_name)
	);
}
