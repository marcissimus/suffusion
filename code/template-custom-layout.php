		...
		if (have_posts()) {
			while (have_posts()) {
				...
				for ($i = 1; $i <= 5; $i++) {
					...
					$sidebar_id = 'sidebar-cl-' . $i;
					if (!suffusion_is_sidebar_empty($sidebar_id)) {
						$default_cols = "suf_clt_wa{$i}_cols";
						$default_height = "suf_clt_wa{$i}_widget_height";
						$default_skinning = "suf_clt_wa{$i}_skin_setting";
						$cols = suffusion_get_post_meta($post->ID, "suf_cpt_wa{$i}_cols", true) ?: ${$default_cols};
						$widget_height = suffusion_get_post_meta($post->ID, "suf_cpt_wa{$i}_widget_height", true) ?: ${$default_height};
						$custom = ${$default_skinning} ? 'custom-skin' : '';
